...
class Showpagocliente extends Component
{
    ...
    public function render()
    {
        $pagocliente = Pago::join('clientes', 'clientes.id', '=', 'pagos.cliente_id')
            ...
            ->paginate($this->cant);
        $pagofiltros = Pago::join('clientes', 'clientes.id', '=', 'pagos.cliente_id')
            ->join('users', 'users.id', 'pagos.user_id')
            ->select(
                'pagos.id as id',
                'pagos.monto as monto',
            ...
            ->get();
        $totalitems = $pagofiltros->count();
        $totalpagado = $pagofiltros->sum('monto');
        return view('livewire.admin.gestionar-reportes.pagocliente.showpagocliente')->with('clientes', $pagocliente)->with('totalitems', $totalitems)->with('totalpagado', $totalpagado);
    }
}
