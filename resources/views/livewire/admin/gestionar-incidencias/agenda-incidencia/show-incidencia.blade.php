<div>
    <div class="mx-auto">
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 content-center">
                ...
                <div class="w-full bg-white rounded-xl overflow-hdden shadow-md p-4 undefined">
                    <div class="flex flex-wrap border-b border-gray-200 undefined">
                        ...
                        <div class="w-full pl-4 max-w-full flex-grow flex-1 mb-2 text-right undefined">
                            <h5 class="text-gray-500 font-light tracking-wide text-base mb-1">N° Incidencias programadas
                                para hoy
                            </h5>
            ...
            <div class="pt-4 px-4 mb-4">
                <div class="w-full bg-white rounded-xl overflow-hdden shadow-md p-4 undefined">
                    <div class="flex flex-wrap border-b border-gray-200 undefined">
                        ...
                        <div class="w-full pl-4 max-w-full flex-grow flex-1 mb-2 text-right undefined">
                            <h5 class="text-gray-500 font-light tracking-wide text-base mb-1">N° Total de Incidencias
                                Programadas
                            </h5>
                            <span class="text-3xl text-gray-900">
                                {{ $totalincidenciasprogramadas }}
                            </span>
                        </div>
                    </div>
                    <div class="text-sm text-gray-700 pt-4 flex items-center undefined"><span
            ...
            @can('admin.gestionarincidencias.agendaincidencias.ver.tipoincidencia')
                    ...
                    <div class="flex flex-wrap border-b border-gray-200 undefined">
                        ...
                        <div class="w-full pl-4 max-w-full flex-grow flex-1 mb-2  undefined">
                            ...
                            <div class="flex justify-end">
                                <span class="">
                                    @include('livewire.admin.gestionar-incidencias.agenda-incidencia.modalcreartipoincidencia')
                                </span>
                                <span class="">
                                    @include('livewire.admin.gestionar-incidencias.agenda-incidencia.modaleditartipoincidencia')
                                </span>
                            </div>
    ...
    <div class=" max-w-7x1 mx-auto px-4 sm:px-6 lg:px-8 ">
        <div class="w-full">
            <div class="bg-white  rounded-2xl px-10 py-8 shadow-lg hover:shadow-2xl transition duration-500 w-full">
                <div class="w-full">
                    <div class=w-full">
                        <h1 class="text-2xl font-semibold leading-tight mb-2">Gestionar Incidencias</h1>
                        @include('livewire.admin.gestionar-incidencias.agenda-incidencia.modalcrear')
                                    ...
                                    <thead>
                                        <tr>
                                            <th wire:click="order('id')"
                                                class="cursor-pointer px-2 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                                                ID
                                                @if ($sort == 'id')
                                                @if ($direction == 'asc')
                                            ...
                                            <th wire:click="order('tipoincidencia')"
                                                class="cursor-pointer px-2 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">
                                                Tipo de Incidencia
                                                <i class="fas fa-sort float-right mt-1"></i>
                                            </th>
                                            ...
                                            <th
                                                class="cursor-pointer px-2 py-3 border-b-2 border-gray-200 bg-gray-100 text-center text-xs font-semibold text-gray-600 uppercase tracking-wider">
                                                Acciones
                                            </th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach ($incidencias as $only)
                                        <tr class="">
                                            ...
                                            <td class="px-2 py-3 border-b border-gray-200 bg-white text-base">
                                                @if ($only->estadonombre == 'Reprogramado')
                                                <span
                                            ...
                                            <td class="px-2 py-3 border-b border-gray-200 text-sm text-center">
                                                @can('admin.gestionarincidencias.agendaincidencias.terminar.incidencia')
                                                <a wire:click="$emit('cambiarestado', {{ $only->id }})"
                                                    class="bg-gradient-to-r from-green-100 via-green-500 to-green-700 btn2 btn-red mb-1 py-2">
                                                    <i class="fas fa-check"></i>
                                                </a>
                                                @endcan
                                                @can('admin.gestionarincidencias.agendaincidencias.reprogramar.incidencia')
                                                <a wire:click="abrirmodalreprogramar({{$only->id}})"
                                                    class="bg-gradient-to-r from-red-100 via-red-500 to-red-700 btn2 btn-red mb-1 py-2">
                                                    <i class="far fa-clock"></i>
                                                </a>
                                                @endcan
                                                @can('admin.gestionarincidencias.agendaincidencias.eliminar.incidencia')
                                                <a wire:click="$emit('deleteincidencia', {{ $only->id }})"
                                                    class="bg-gradient-to-r from-red-100 via-red-500 to-red-700 btn2 btn-red mb-1 py-2">
    ...
    @include('livewire.admin.gestionar-incidencias.agenda-incidencia.modalreprogramarincidencia')
    <script>
        livewire.on('deleteincidencia', thisid => {
            Swal.fire({
                title: 'Estás seguro?',
                text: "¡Eliminaras esta incidencia!",
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    @this.call('deleteincidencia', thisid)
                    Swal.fire(
                        'Actualizado!',
    ...
    <script>
        livewire.on('cambiarestado', thisid => {
            Swal.fire({
                title: 'Estás seguro?',
                text: "¡Marcaras esta incidencia como terminada!",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                ...
                if (result.isConfirmed) {
                    @this.call('confirmarincidenciaterminada', thisid)
                    Swal.fire(
                        'Actualizado!',
                        'La incidencia ha sido actualizada.',
                        'success'
                    )
                }
            })
    ...
    <script>
        livewire.on('deletethis', thisid => {
            Swal.fire({
                title: 'Estás seguro?',
                text: "Eliminaras este tipo de incidencia!",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                ...
                if (result.isConfirmed) {
                    @this.call('deletetipoincidencia', thisid)
                    Swal.fire(
                        'Actualizado!',
                        'El Tipo de Incidencia ha sido eliminado con éxito.',
                        'success'
                    )
                }
            })
    ...
    <script>
        livewire.on('reprogramarthis', thisid => {
            Swal.fire({
                title: 'Estás seguro?',
                text: "Reprogramaras la fecha de visita para esta incidencia!",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                ...
                if (result.isConfirmed) {
                    @this.call('reprogramarthisincidencia', thisid)
                    Swal.fire(
                        'Actualizado!',
                        'El Incidencia se reprogramó con éxito.',
                        'success'
                    )
                }
            })
