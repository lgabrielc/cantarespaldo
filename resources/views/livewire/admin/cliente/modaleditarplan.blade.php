...
<x-jet-dialog-modal wire:model='vermodaleditarplan'>
    ...
    <x-slot name="content">
        ...
        <div class="mb-4">
            ...
            <x-jet-input type="text" class="block w-full px-6 border py-1 mt-1" placeholder="Ejm: Plan Premium"
        ...
        <div class="mb-4">
            ...
            <x-jet-input type="text" class="block w-full px-6 border py-1 mt-1" placeholder="Ejm: 10MB"
                wire:model="descarga" disabled={{$disabled2}} />
            @error('descarga')
            <div class="text-red-500">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-4">
            ...
            <x-jet-input type="text" class="block w-full px-6 border py-1 mt-1" placeholder="Ejm: 10MB"
                wire:model="subida" disabled={{$disabled2}} />
            @error('subida')
            <div class="text-red-500">{{ $message }}</div>
            @enderror
        </div>
        <div class="mb-4">
            ...
            <x-jet-input type="text" class="block w-full px-6 border py-1 mt-1" placeholder="Ejm: 60.00"
                wire:model="precio" disabled={{$disabled2}} />
            @error('precio')
            <div class="text-red-500">{{ $message }}</div>
            @enderror
        </div>
    ...
    <x-slot name="footer">
        ...
        @if ($planusado == 0 && is_numeric($plan))
        @can('admin.clientes.eliminar.plan')
        <x-jet-danger-button wire:click="$emit('delethisplan',{{$plan}})" wire:loading.attr="disabled">
            {{ __('Eliminar') }}
        </x-jet-danger-button>
        @endcan
        @endif
        @can('admin.clientes.editar.plan')
